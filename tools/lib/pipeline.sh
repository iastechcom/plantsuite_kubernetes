...
_should_skip_infra() {
  local component="$1"
  local skip
  for skip in "${GATEWAY_INFRA_NEVER[@]:-}" "${GATEWAY_INFRA_SKIP[@]:-}"; do
...
build_real_pipeline() {
  ...
  _should_skip_infra "metrics-server" || { REAL_STEP_IDS+=("metrics-server"); REAL_STEP_LABELS+=("metrics-server"); }
  ...
  _should_skip_infra "aspire" || { REAL_STEP_IDS+=("aspire"); REAL_STEP_LABELS+=("aspire"); }
  ...
  if ! _should_skip_infra "postgresql"; then
  ...
  _should_skip_infra "redis" || { REAL_STEP_IDS+=("redis"); REAL_STEP_LABELS+=("redis"); }

  if ! _should_skip_infra "keycloak"; then
  ...
  if ! _should_skip_infra "rabbitmq"; then
  ...
  _should_skip_infra "vernemq" || { REAL_STEP_IDS+=("vernemq"); REAL_STEP_LABELS+=("vernemq"); }
